...
public function __construct( $file ){
	...
	// Model
	self::$model = new WaterMainSliderModel();
	...
	// Shortcodes
    add_shortcode('water_main_slider', array(__CLASS__, 'replace_shortcode') );
}
...
function activate(){
	...
	if ($wpdb->get_var("SHOW TABLES LIKE '" . WATER_MAIN_SLIDER_DB_TABLE_NAME . "'") != WATER_MAIN_SLIDER_DB_TABLE_NAME)
	{
		$sql = "CREATE TABLE " . WATER_MAIN_SLIDER_DB_TABLE_NAME . " (
		...
		// Назначение dbDelta: создание и обновление таблицы
      	...
      	add_option(WATER_MAIN_SLIDER_PLUGIN_NAME . "_db_version", WATER_MAIN_SLIDER_PLUGIN_DB_VERSION);
	}
}
...
static function register_plugin_button_in_admin_menu(){
	...
	add_menu_page(
		WATER_MAIN_SLIDER_PLUGIN_NAME_RU, 						// содержимое <title>
		WATER_MAIN_SLIDER_PLUGIN_NAME_RU,							// название пункта в меню
		'manage_options',										// уровень доступа (взял из примера)
		WATER_MAIN_SLIDER_PLUGIN_NAME,							// URL страницы с плагином
		array(__CLASS__, 'render_admin_page'), 					// функция, генерирующая страницу
		plugins_url( WATER_MAIN_SLIDER_PLUGIN_NAME . '/static/images/admin-menu-button.png' ) // адрес иконки
	);
}
...
static function render_admin_page(){
	if (is_admin()) {
		if ( (isset($_GET['page'])) && ($_GET['page'] == WATER_MAIN_SLIDER_PLUGIN_NAME) ) {
	...
	if (is_admin()) {
		if ( (isset($_GET['page'])) && ($_GET['page'] == WATER_MAIN_SLIDER_PLUGIN_NAME) ) {
			...
			case 'add':
				...
				self::$model->save();
				print('<script>window.location = "' . WATER_MAIN_SLIDER_PLUGIN_ADMIN_URL . '"</script>');
				break;
			case 'edit':
				self::$model->get( $id );
				self::$model->title		= $title;
...
static function enqueue_static() {
	wp_enqueue_style(WATER_MAIN_SLIDER_PLUGIN_NAME . '_style', plugins_url( WATER_MAIN_SLIDER_PLUGIN_NAME . '/static/css/style.css' ));
}
 ...
 */
	...
	static function enqueue_plugin_scripts($plugin_array)
	{
	    ...
	    $plugin_array["management_button_plugin"] = plugins_url( WATER_MAIN_SLIDER_PLUGIN_NAME . '/static/js/modify_tiny_mce.js' );
	    return $plugin_array;
	}
	/**
...
static function media_selector_print_scripts() {
	$my_saved_attachment_post_id = get_option( WATER_MAIN_SLIDER_PLUGIN_NAME.'media_selector_attachment_id', 0 );
...
static function is_this_plugin_admin_page() {
	return is_admin() && ((isset($_GET['page'])) && ($_GET['page'] == WATER_MAIN_SLIDER_PLUGIN_NAME));
}
